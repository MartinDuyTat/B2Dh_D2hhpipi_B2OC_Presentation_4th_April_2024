...
\begin{frame}{Motivation for this analysis}
  ...
  \begin{itemize}
    ...
    \item{Strong-phase analyses of both $D\to K^+K^-\pi^+\pi^-$ (Oxford) and $D\to\pi^+\pi^-\pi^+\pi^-$ (Oxford-USTC) are emerging from BESIII}
    \begin{itemize}
      \item{For $D\to\pi^+\pi^-\pi^+\pi^-$, these improve in precision on earlier study made with CLEO-c data \href{https://link.springer.com/article/10.1007/JHEP01(2018)144}{JHEP \textbf{01} (2018) 144}}
    \end{itemize}
    \item{Use this input to make first model-independent measurement with $D\to K^+K^-\pi^+\pi^-$, updating earlier LHCb model-dependent analysis}
    \item{Use same 4-body selection to clone analysis for $D\to\pi^+\pi^-\pi^+\pi^-$, again using BESIII inputs}
    \item{After surveying compatibility of results, perform joint model-independent analysis of both modes}
  \end{itemize}
\end{frame}
...
\begin{frame}{BESIII preliminary $D^0\to\pi^+\pi^-\pi^+\pi^-$ strong-phase results}
  ...
  \begin{itemize}
    ...
    \item{New BESIII analysis uses a new and optimised binning scheme}
    \begin{itemize}
      \item{Published BESIII amplitude model constructed from a larger data set}
...
\begin{frame}{Global fit}
  ...
  \begin{itemize}
    ...
    \item{We \underline{already have} tuples of $B^\pm\to[\pi^+\pi^-\pi^+\pi^-]_Dh^\pm$ from phase-space integrated measurement}
    \begin{enumerate}
      \setlength\itemsep{0.5em}
